...
class ProSafeLinux:
    ...
    def recv(self, maxlen=8192):
        ...
        self.ssocket.settimeout(self.timeout)
        try:
            message, address = self.ssocket.recvfrom(maxlen)
    ...
    def parse_data(self, pack):
        ...
        while pos < len(pack):
            ...
            if cmdlen > 0:
                ...
                value = None
            if cmd in data and value is not None:
    ...
    def transmit(self, cmddict, mac):
        ...
        self.send(ipadr, self.SENDPORT, data)
        message, address = self.recv_all()
        while message is None and transmit_counter < 3:
            time.sleep(1)
            message, address = self.recv_all()
            transmit_counter += 1
        if message is None:
